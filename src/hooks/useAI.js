...
// Konsumeras av: QuestCard.jsx, QuestGrid.jsx, AICoach.jsx
// ═══════════════════════════════════════════════════════════════
...
function buildCoachPrompt(m, c) {
  const completedCats = Object.entries(c.categoryCount || {})
    ...
    .map(([k]) => k)
    .join(', ');
  const pending = 0; // QuestGrid beräknar detta — coachen får approximationen
...
Sektionens läge: Album II ute 1 mars 2026. Individuella kanaler aktiveras. Japan X-konto aktivt. Spotify: 110 followers. Instagram: 209 followers.

Rolltyp: ${rtLabel} — ${rtDesc}
VIKTIGT: Anpassa quests strikt till rolltypen.
- amplifier: sociala medier, storytelling, synlighet OK
- enabler: INGA sociala medier-quests — fokus på infrastruktur, teknik, produktion
...
7. LEDARSKAPSSIGNAL: Om roleDrain är tomt/vagt/under 10 ord — skapa direktiva, tydliga, strukturerade quests.
...
function buildSidequestPrompt(m, c) {
  return `Du är AI-coach för ${m.name} i Sektionen, ett indie-band från Göteborg i Operation POST II — på väg från ideell till professionell verksamhet. EP klar till juli 2026.
...
Skapa 3 sidequest-förslag. Sidequests är lekfulla, kreativa, korta. De kan vara:
- Spela in en TikTok
- Öva ett instrument utan mål i 30 min
- Skriva ett låtfragment
- Lyssna aktivt på ett referensalbum
- Ringa någon i bandet och prata musik
- Något helt annat som passar deras rolltyp

XP ska vara lågt (25–50). Tonen ska vara inbjudande, aldrig pressande.
...
Deras profil:
- Motivation: "${c.motivation || 'ej angiven'}"
- Styrka: "${c.roleEnjoy || 'ej angiven'}"
- Utmaning: "${c.roleDrain || 'ej angiven'}"
...
/**
 * useAI()
 * Hook med alla AI-funktioner bundna till store-state.
 */
export function useAI() {
  ...
  // ── aiValidate ──────────────────────────────────────────────
  // Validerar ett quest-svar via AI och tilldelar XP proportionellt.
  // Sätter q.aiVerdict och q.aiThinking på quest-objektet.
  async function aiValidate(q, desc, event) {
    const m = MEMBERS[S.me];
    const c = S.chars[S.me];
    if (!m || !c) return;

    // Optimistisk UI: markera quest som "thinking"
    setState(prev => ({
      ...prev,
      quests: prev.quests.map(sq =>
        sq.id === q.id ? { ...sq, aiThinking: true } : sq
      ),
    }));
    ...
    try {
      ...
      setState(prev => ({
        ...
        quests: prev.quests.map(sq =>
          sq.id === q.id
            ? { ...sq, aiThinking: false, aiVerdict: { text: `${feedback} — ${score}/100 → ${xpEarned} XP`, cls } }
            : sq
        ),
      }));

      if (verdict !== 'rejected' && xpEarned > 0) {
        setTimeout(() => awardXP(q, xpEarned, event), 1200);
      }
    } catch {
      setState(prev => ({
        ...prev,
        quests: prev.quests.map(sq =>
          sq.id === q.id
            ? { ...sq, aiThinking: false, aiVerdict: { text: 'AI ej tillgänglig — halvt XP tilldelat', cls: 'v-partial' } }
            : sq
        ),
      }));
      setTimeout(() => awardXP(q, Math.round(q.xp * 0.5), event), 800);
    }
  }

  // ── generatePersonalQuests ──────────────────────────────────
  // Genererar 4 personliga AI-quests baserade på rollkalibrering.
  // refreshMode=true: används från "Nya quests →"-knappen.
  async function generatePersonalQuests(refreshMode = false) {
    const m = MEMBERS[S.me];
    const c = S.chars[S.me];
    if (!m || !c) return;
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        quests: [
          // I refresh-läge: ta bort gamla completade personal quests
          ...(refreshMode
            ? prev.quests.filter(q => !(q.owner === S.me && q.personal && q.type !== 'ghost' && q.done))
            : prev.quests),
          ...newQuests,
        ],
      }));
    } catch {
      // Fallback: en minimal quest om AI misslyckas
      const fallback = {
        id: 401, owner: S.me,
        ...
        cat: 'wisdom', xp: 75, stars: '', region: '🌐 Personal',
        recur: 'none', type: 'strategic', done: false, aiVerdict: null, personal: true,
      };
      setState(prev => ({ ...prev, quests: [...prev.quests, fallback] }));
    }
  }

  // ── refreshCoach ────────────────────────────────────────────
  // Hämtar en personlig coaching-insikt och returnerar texten.
  // UIen (AICoach.jsx) ansvarar för att visa loading-state.
  ...
  // Lägger till en ghost quest i state om villkoren är uppfyllda.
  async function checkGhostQuest() {
    const c = S.chars[S.me];
    const m = MEMBERS[S.me];
    if (!c || !m) return;
    ...
    // Trigga inte om < 7 dagar eller om ghost quest redan finns aktiv
    ...
    setState(prev => ({ ...prev, quests: [...prev.quests, ghostQuest] }));
  }

  // ── showSidequestNudge ──────────────────────────────────────
  // Genererar 3 sidequest-förslag och returnerar dem som array.
  // WeeklyCheckout.jsx (eller SidequestNudge.jsx) renderar listan.
  // weekKey används för att markera veckan som "sidequested".
  async function showSidequestNudge(weekKey) {
    const m = MEMBERS[S.me];
    const c = S.chars[S.me];
    if (!m || !c) return [];
    ...
    try {
      ...
      const parsed = parseJSON(txt);
      return Array.isArray(parsed) ? parsed : fallbacks;
    } catch {
      return fallbacks;
    }
  }
