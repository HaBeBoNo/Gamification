...
| Presence | Supabase `member_presence` | Zustand presence slice in [src/state/store.ts](/Users/t-nab/Documents/Gamification/src/state/store.ts) | [src/hooks/usePresenceSync.ts](/Users/t-nab/Documents/Gamification/src/hooks/usePresenceSync.ts), [src/lib/socialData.ts](/Users/t-nab/Documents/Gamification/src/lib/socialData.ts) | Home, ActivityFeed, coach context | Server-owned ephemeral state. Client slice is a reactive projection of active presence rows, not business history. |
...
- Client code may project `member_presence` into Zustand for live UX, but that projection is a cache of server truth rather than a second source of truth.
